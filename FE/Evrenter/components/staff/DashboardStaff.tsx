...
import { api } from "@/lib/api"   // ✅ dùng instance có token

const statsDataDefault = [
...
export function Dashboard() {
  ...
  const [statsData, setStatsData] = useState(statsDataDefault)
  ...
  useEffect(() => {
    const fetchStation = async () => {
      try {
        ...
        const availableVehicles = data.vehicles?.filter((v: any) => v.status === "AVAILABLE").length || 0

        setStatsData((prev) =>
          prev.map((item) =>
            item.title === "Tổng số xe"
              ? {
                  ...item,
                  value: totalVehicles.toString(),
                  change: "",
                  changeType: "increase",
                  description: `Xe đang có sẵn: ${availableVehicles}`,
                }
              : item
          )
        )
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground">
              Tổng quan hệ thống thuê xe {station ? `— ${station.stationName}` : ""}
            </p>
          </div>
