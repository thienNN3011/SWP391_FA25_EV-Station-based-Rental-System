...
//        User user=userRepository.findByUsernameAndStatus(username,"PENDING");
    ...
    public List<UserListResponse> showAllUsers(String username) {
        ...
        } else if ("STAFF".equals(requestUser.getRole())) {
            userList = userRepository.findAll().stream().filter(user -> "RENTER".equals(user.getRole())).collect(Collectors.toList());
        } else {
            throw new BusinessException("Không có quyền truy cập");
        }
        if (userList.isEmpty()) {
            throw new ResourceNotFoundException("Không có user nào trong hệ thống");
        }
